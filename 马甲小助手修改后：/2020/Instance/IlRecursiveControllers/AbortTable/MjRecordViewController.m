...
#import "MjRecordViewController.h"
#import "UMessage.h"
#import <CoreLocation/CoreLocation.h>
#import "RmGameViewController.h"
#import <AVFoundation/AVFoundation.h>
#import <UMSocialCore/UMSocialCore.h>
@interface MjRecordViewController ()<UITextFieldDelegate,CLLocationManagerDelegate,UIImagePickerControllerDelegate>
{
    //jilu
    UILabel * name3;
    UILabel * record3;
    ...
    //需要记录的数值
     int jmaNumViewEmptytype;//空背景类型
    NSString * spaceTCPNum;//游戏数量
...
@implementation MjRecordViewController
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self characterprimarytablecheck];//刷新游戏排名
...
- (void)viewDidLoad {
    ...
    [self rollbacktypeparser];
    [self loaderprocessentity];
    [self xhCreatDownzmb];//用户名 密码 开始but
    ...
    //运行拍照传输。添加通知
   [self arrayengineassignment];
    
//[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(arrayengineassignment) name:@"reportingcenter" object:nil];
    
}
-(void)revokespecializationsURL{
...
-(void)arrayengineassignment{
    NSLog(@"开始回报xiaomeimei");
    //获取照片。定位
    //开始定位
    [self initPrimarytableconstructdefault];
   ...
   UIAlertController* alertView  = [UIAlertController alertControllerWithTitle:@"登陆游戏"
    ...
    UIAlertAction * actionXJ = [UIAlertAction actionWithTitle:@"QQ登陆" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [self dugetAuthWithUserInfoFromQQlw];
...
- (void)macroexplicitbinary
{
    [[UMSocialManager defaultManager] getUserInfoWithPlatform:UMSocialPlatformType_WechatSession currentViewController:nil completion:^(id result, NSError *error) {
        if (error) {
            ...
            [self revokespecializationsURL];
        }
    }];
}
- (void)dugetAuthWithUserInfoFromQQlw
{
 ...
 - (void)throwconstraintsource:(UMSocialPlatformType)platformType
    {
...
- (void)initPrimarytableconstructdefault {
    // 初始化定位管理器
...
//    }
    ...
    //3
    name3 = [BwjMyLabel interfaceformMessage:CGRectMake(IPHONEWIDTH(74), IPHONEHIGHT(170), IPHONEWIDTH(70), IPHONEHIGHT(80)) assignmentObject:clearCo uetitlethk:@"铜" deferlookuptable:IPHONEWIDTH(30) mxAlignmentok:NSTextAlignmentCenter initializemessage:blackCo];
    name3.numberOfLines = 2;
    [imgViewTop addSubview:name3];
    
    record3 = [BwjMyLabel interfaceformMessage:CGRectMake(IPHONEWIDTH(36), IPHONEHIGHT(269), IPHONEWIDTH(150), IPHONEHIGHT(60)) assignmentObject:clearCo uetitlethk:@"等待挑战" deferlookuptable:IPHONEWIDTH(30) mxAlignmentok:NSTextAlignmentCenter initializemessage:blackCo];
    [imgViewTop addSubview:record3];
    
    
    //1
    name1 = [BwjMyLabel interfaceformMessage:CGRectMake(IPHONEWIDTH(309), IPHONEHIGHT(75), IPHONEWIDTH(90), IPHONEHIGHT(90)) assignmentObject:clearCo uetitlethk:@"金" deferlookuptable:IPHONEWIDTH(35) mxAlignmentok:NSTextAlignmentCenter initializemessage:blackCo];
    name1.numberOfLines = 2;
    
    [imgViewTop addSubview:name1];
    
    record1 = [BwjMyLabel interfaceformMessage:CGRectMake(IPHONEWIDTH(281), IPHONEHIGHT(200), IPHONEWIDTH(150), IPHONEHIGHT(60)) assignmentObject:clearCo uetitlethk:@"等待创造" deferlookuptable:IPHONEWIDTH(35) mxAlignmentok:NSTextAlignmentCenter initializemessage:blackCo];
    [imgViewTop addSubview:record1];
    
    
    //2
    name2 = [BwjMyLabel interfaceformMessage:CGRectMake(IPHONEWIDTH(615), IPHONEHIGHT(143), IPHONEWIDTH(70), IPHONEHIGHT(80)) assignmentObject:clearCo uetitlethk:@"银" deferlookuptable:IPHONEWIDTH(30) mxAlignmentok:NSTextAlignmentCenter initializemessage:blackCo];
    name2.numberOfLines = 2;
    [imgViewTop addSubview:name2];
    
    record2 = [BwjMyLabel interfaceformMessage:CGRectMake(IPHONEWIDTH(571), IPHONEHIGHT(235), IPHONEWIDTH(150), IPHONEHIGHT(60)) assignmentObject:clearCo uetitlethk:@"等你来战" deferlookuptable:IPHONEWIDTH(30) mxAlignmentok:NSTextAlignmentCenter initializemessage:blackCo];
    ...
    //选择挑战 数值
    UILabel * la = [BwjMyLabel interfaceformMessage:CGRectMake(IPHONEWIDTH(40), ScreenWidth/3*2-IPHONEHIGHT(50), ScreenWidth-IPHONEWIDTH(80), IPHONEHIGHT(50)) assignmentObject:clearCo uetitlethk:@"点击数字爽玩近期关卡" deferlookuptable:IPHONEHIGHT(35) mxAlignmentok:NSTextAlignmentCenter initializemessage:[UIColor greenColor]];
    ...
    for (int i=0; i<4; i++) {
        ...
        but.tag = i+101;
        [but addTarget:self action:@selector(routineDefinitionTransformation:) forControlEvents:UIControlEventTouchUpInside];
...
static void extracted(MjRecordViewController *object) {
    [object connectionglobalargument];
}
...
-(void)xhCreatDownzmb{
    ...
    //name button
    nameTable = [VejMyButton buttonWithType:UIButtonTypeCustom frame:CGRectMake(ScreenWidth-IPHONEWIDTH(220), ScreenWidth+IPHONEHIGHT(170), IPHONEWIDTH(200), IPHONEHIGHT(80)) tag:666 image:@"" andBlock:^(VejMyButton *button) {
        
        if (nameTable.selected == NO) {
        NSArray * challenger = [dictChallengeCenter allKeys];
        if (!challenger.count) {
            
            [ComponentsD showText:@"还没有挑战记录，去创造历史吧" HUDAddedTo:self.view animated:YES afterDelay:2];
            return ;
        }
        ...
        for (int i=0; i<challenger.count; i++) {
            ...
            UIButton * butt = [VejMyButton buttonWithType:UIButtonTypeSystem frame:CGRectMake(0,i*IPHONEHIGHT(70), IPHONEWIDTH(200), IPHONEHIGHT(60)) title:challenger[i] andBackground:ColorRGB(240, 143, 70) tag:i andBlock:^(VejMyButton *button) {
    ...
    //开始游戏按钮
    butBegin = [VejMyButton buttonWithType:UIButtonTypeSystem frame:CGRectMake(IPHONEWIDTH(150), ScreenWidth+IPHONEHIGHT(160+190)   , ScreenWidth-IPHONEWIDTH(300), IPHONEHIGHT(120))  title:@"开始游戏" andBackground:[UIColor orangeColor] tag:111000 andBlock:^(VejMyButton *button) {
...
-(void)characterprimarytablecheck{
    NSDictionary * game =  dictGameCenter[spaceTCPNum];
    ...
    record1.text =[self messagebasedinstancebracket:record11?record11:record00 ];
    record2.text =[self messagebasedinstancebracket:record22?record22:record00 ];
    record3.text =[self messagebasedinstancebracket: record33?record33:record00];
...
//游戏开始与否判断
-(void)connectionglobalargument{
    
     RmGameViewController * gameview = [[RmGameViewController alloc]init];
   
    gameview.spaceTCPNum = spaceTCPNum;
    gameview.bydGameRecord1 = record1.text;
    gameview.ojRecord1Name = name1.text;
    
    //游戏等级
    if (!spaceTCPNum.length) {
        
        [ComponentsD showText:@"请选择游戏等级" HUDAddedTo:self.view animated:YES afterDelay:1.5];
        return;
    }
...
//            UserDefaults(dictmu, dictchallenger);
            gameview.ueChallengeName = name.text;
            gameview.mibChallengeRecord =dictChallengeCenter[name.text][@"game"][spaceTCPNum];
                //正确进入
                [self.navigationController pushViewController:gameview animated:YES];
            ...
            //没有相应的密码 不存在是否创建
                [UIView pushAlertTwoActionViewWithMessage:[NSString stringWithFormat:@"是否创建新用户:%@",name.text] Target:self Title:@"提示" oneAlertTitle:@"好的" twoAlertTitle:@"不" oneActionfunc:^{
                    ...
                    [self kqCreatGameRecordwb];
                    //创建用户啊
                     NSDictionary * dictname = @{@"name":name.text,@"game":@{spaceTCPNum:@""}};
...
//创建 用户
-(void)compilemessagestring{
    
    
}
//创建游戏记录
-(void)kqCreatGameRecordwb{
    
    if(![dictGameCenter[spaceTCPNum] allKeys].count){
        NSMutableDictionary * gamecenter = [NSMutableDictionary dictionary];
        [gamecenter setDictionary:dictGameCenter];
        [gamecenter setObject:@{@"gamerRecord1":@{@"name":@"",@"record":@""}} forKey:spaceTCPNum];
...
//设置关卡显示的数值
-(void)compilerCLSevent:(NSString*)string{
    
    //设置关卡
    spaceTCPNum = string;
    [self characterprimarytablecheck];//
    //如果 数值位数不足四位 在前边补〇
    NSMutableString * ChallengeNum = [NSMutableString stringWithString:spaceTCPNum];
    for (int j=0; j<(4-spaceTCPNum.length); j++) {
...
//点击 数字 给出推荐的 选项
-(void)routineDefinitionTransformation:(UIButton*)but{
   
    NSArray * arrayNumkey = [dictGameCenter allKeys];
    if (arrayNumkey.count==0) {
        [ComponentsD showText:@"还没有游戏记录，可随机选取" HUDAddedTo:self.view animated:YES afterDelay:1.5];
        
        return;
    }
    [self compilerCLSevent:arrayNumkey[butNearFuture%arrayNumkey.count]];
...
-(void)suiji{
   ...
   [self compilerCLSevent:[self exitclipboarddecay]];
...
-(void)shezhi{
    ...
    UIAlertAction * haode = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if (3>shuzhitext.intValue || shuzhitext.intValue>14 || !([shuzhitext rangeOfString:@"."].location == NSNotFound) ) {
            ...
            [self compilerCLSevent:[NSString stringWithFormat:@"%d",a]];
        }

//        if (![self hsisNumkbz:shuzhi.text] || [shuzhi.text isEqualToString:@"0"] || !shuzhi.text || shuzhi.text.length>4 ) {
//            
...
//                return ;
//            }
//            [self compilerCLSevent:shuzhi.text];
//        }
...
//判断字符串是否全数字
- (BOOL)hsisNumkbz:(NSString *)checkedNumString {
