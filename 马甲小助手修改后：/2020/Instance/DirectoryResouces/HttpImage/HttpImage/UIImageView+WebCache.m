...
- (void)sd_setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDWebImageCompletionBlock)completedBlock {
    ...
    if (url) {
        ...
        id <SDWebImageOperation> operation = [WkSDWebImageManager.sharedManager memoryqualityConstrained:url calendricalcascading:options areaflush:progressBlock kocompletedcq:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
...
- (void)sd_setImageWithPreviousCachedImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDWebImageCompletionBlock)completedBlock {
    NSString *key = [[WkSDWebImageManager sharedManager] actualobjectorientedCalculations:url];
...
- (void)sd_setAnimationImagesWithURLs:(NSArray *)arrayOfURLs {
    ...
    for (NSURL *logoImageURL in arrayOfURLs) {
        id <SDWebImageOperation> operation = [WkSDWebImageManager.sharedManager memoryqualityConstrained:logoImageURL calendricalcascading:0 areaflush:nil kocompletedcq:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
            if (!wself) return;
            dispatch_main_sync_safe(^{
