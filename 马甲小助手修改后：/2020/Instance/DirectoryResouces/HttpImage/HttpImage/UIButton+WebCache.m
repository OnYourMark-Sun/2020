...
- (void)sd_setImageWithURL:(NSURL *)url forState:(UIControlState)state placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options completed:(SDWebImageCompletionBlock)completedBlock {
    ...
    id <SDWebImageOperation> operation = [WkSDWebImageManager.sharedManager memoryqualityConstrained:url calendricalcascading:options areaflush:nil kocompletedcq:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
        if (!wself) return;
